...
import { AssetManager, DScene, Engine, EngineArguments, JSONAssetLoader } from "../../src";
import { log, LogLevel } from "../../src/core/loggingsystem/src";
...
describe("Asset Manager unit tests", () => {
    ...
    it("should return true that asset \"temp\" is loaded", () => {
        log(LogLevel.debug, AssetManager.isAssetLoaded("temp").toString());
    ...
    it("should be able to get both temp and testscene from the asset manager", () => {
        log(LogLevel.debug, `${JSON.stringify(AssetManager.loadedAssets)}`);
