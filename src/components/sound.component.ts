...
export class SoundComponent extends Component {
    ...
    constructor(...args: [ISound]) {
        ...
        this._sounds.forEach((sound) => {
            this.registerSound(sound.url, sound.action);
        });
    }
    /**
     ...
     * @returns boolean
     */
    private registerSound(url: string, action: SoundAction): boolean {
        this.sendMessage(JSON.stringify({url, action}), MessageType.Sound, true);
